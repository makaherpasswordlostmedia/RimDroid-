...
void rimdroid_zfa_swap(void) {
    ...
    p_zfaFlushFront();
    // Do NOT reset g_zfa_context_bound here.
    // zfaFlushFront() presents the current swapchain image but does NOT release
    // the context binding — the next frame can (and should) keep rendering into
    // the same binding without re-calling zfaMakeCurrent().  Resetting the flag
    // here caused zfaMakeCurrent() to be called on every SDL_GL_MakeCurrent after
    // each swap, re-acquiring a new swapchain image mid-frame.  During the
    // Unity fullscreen transition (surfaceDestroyed → surfaceCreated) this raced
    // with the Android Surface replacement: the re-acquire landed on the old
    // (destroyed) surface, the subsequent surfaceCreated set is_dirty=true but
    // SDL_GL_MakeCurrent was never called again → the new surface was never
    // bound → black screen for the rest of the session.
    // The only correct trigger for a forced rebind is is_dirty (set by
    // rimdroid_surface_init on surfaceCreated), which rimdroid_zfa_make_current
    // already handles.
}
